...
public class MatrixPanel extends JPanel {
    ...
    private void zoom(MouseWheelEvent e) {
        if (e.getScrollType() == MouseWheelEvent.WHEEL_UNIT_SCROLL) {
            ...
            scroll.getHorizontalScrollBar().setValue((int) (dx * (scroll.getHorizontalScrollBar().getMaximum() - scroll.getHorizontalScrollBar().getVisibleAmount()) / ((double) colors.length * ((iScale / scale) - 1))));
            scroll.getVerticalScrollBar().setValue((int) (dy * (scroll.getVerticalScrollBar().getMaximum() - scroll.getVerticalScrollBar().getVisibleAmount()) / ((double) colors[0].length * ((iScale / scale) - 1))));
